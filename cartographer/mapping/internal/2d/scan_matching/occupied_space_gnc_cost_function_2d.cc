...
class OccupiedSpaceGncCostFunction2D {
public:
  OccupiedSpaceGncCostFunction2D(const double scaling_factor,
  ...
  bool operator()(const T* const pose, T* residual) const {
    ...
    for (size_t i = 0; i < point_cloud_.size(); ++i) {
      ...
      residual[i] = scaling_factor_ * residual[i] *
          T(gnc_state->get_weights()->at(i));
      if (max_residual < residual[i]) {
        max_residual = residual[i];
      }
      gnc_state->set_distance(i, GetScalarFromJet(residual[i]) /
                              gnc_state->get_weights()->at(i));
    }
    gnc_state->set_max_residual(GetScalarFromJet(max_residual));
    return true;
  }
  ...
  private:
    ...
    OccupiedSpaceGncCostFunction2D(
        const OccupiedSpaceGncCostFunction2D&) = delete;
    OccupiedSpaceGncCostFunction2D& operator=(
        const OccupiedSpaceGncCostFunction2D&) = delete;
...
ceres::CostFunction* CreateOccupiedSpaceGncCostFunction2D(
  ...
  return new ceres::AutoDiffCostFunction<OccupiedSpaceGncCostFunction2D,
      ceres::DYNAMIC /* residuals */, 3 /* pose variables */>(
          new OccupiedSpaceGncCostFunction2D(
              scaling_factor, point_cloud, grid, gnc_state), point_cloud.size());
}
